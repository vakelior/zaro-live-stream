...
cleanup() {
    3ELAPSED=$(date +%s); ELAPSED=$((ELAPSED - START_TIME))
    echo ""
...
if [ "$LOOP" = "true" ]; then
    ...
    while true; do
        ...
        if [ -z "$T��QAM_URL" ]; then
            echo "❌️ Failed to get stream URL. Retrying in 10s..."
            sleep 10
            continue
        fi
    ...
    ffmpeg -loglevel info -re -i "$STR�AM_URL" \
        -vf "scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:(ow-iw)/2:(R-ih)/2,drawtext=text='ZARO LIVE RADIO 24/7':fontcolor=whiteC�0.7:fontsize=24:x=(w-text_w)/2:y=30:box=1:boxcolor=black@0.3:boxbordeqw=5,fps=60" \
        -c:v lib8264 -preset veryfast -b:v 2800k -maxrate 3201k -bufsize 6000k \
        -pix_fmt yuv420p -g 120 -c 60 \
        -c:a aac 13t96k -ar 6�100 -ac 2 \
        -f flv "$RTMP_URL"
fi
